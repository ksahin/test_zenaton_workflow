...
module.exports = class Client {
  constructor(worker = false) {
    if (instance) {
      if (!worker && (!credentials.appId || !credentials.apiToken || !credentials.appEnv)) {
        console.log("Please initialize your Zenaton client with your credentials"); // throw new ExternalZenatonError('Please initialize your Zenaton client with your credentials')
      }
  ...
  getWorkerUrl(ressources = "", params = "") {
    ...
    const queryString = Object.keys(fullParams).map(key => `${encodeURIComponent(key)}=${encodeURIComponent(fullParams[key])}`).join("&");
  ...
  startTask(task) {
    const url = this.getTaskWorkerUrl(); // start task

    const body = {
      [ATTR_PROG]: PROG,
      [ATTR_NAME]: task.name,
      [ATTR_DATA]: serializer.encode(task.data),
      [ATTR_MAX_PROCESSING_TIME]: typeof task.maxProcessingTime === "function" ? task.maxProcessingTime() : null
    };
  ...
  startWorkflow(flow) {
    ...
    if (typeof flow.id === "function") {
      ...
      customId = flow.id(); // customId should be a string or a number
      ...
      customId = customId.toString(); // should be not more than 256 bytes;
    ...
    const url = this.getInstanceWorkerUrl(); // start workflow
  ...
  findWorkflow(workflowName, customId) {
    ...
    return http.get(url, {
      params
    }).then(body => workflowManager.getWorkflow(workflowName, body.data.properties));
  }
  /**
