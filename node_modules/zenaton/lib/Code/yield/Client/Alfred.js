...
const Alfred = class Alfred {
  ...
  runTask(task) {
    ...
    const promise = this._request(endpoint, mutation, variables).then(res => res.dispatchTask);
  ...
  scheduleTask(task) {
    ...
    const promise = this._request(endpoint, mutation, variables).then(res => res.createTaskSchedule);
  ...
  runWorkflow(w) {
    ...
    const promise = this._request(endpoint, mutation, variables).then(res => res.dispatchWorkflow);
  ...
  scheduleWorkflow(w) {
    ...
    const promise = this._request(endpoint, mutation, variables).then(res => res.createWorkflowSchedule);
  ...
  terminateWorkflow(query) {
    ...
    const promise = this._request(endpoint, mutation, variables).then(res => res.killWorkflows);
  ...
  pauseWorkflow(query) {
    ...
    const promise = this._request(endpoint, mutation, variables).then(res => res.pauseWorkflows);
  ...
  resumeWorkflow(query) {
    ...
    const promise = this._request(endpoint, mutation, variables).then(res => res.resumeWorkflows);
  ...
  sendEvent(query, eventName, eventData) {
    ...
    const promise = this._request(endpoint, mutation, variables).then(res => res.sendEventToWorkflows);
  ...
  sendEventByInstanceId(id, eventName, eventData) {
    ...
    const promise = this._request(endpoint, mutation, variables).then(res => res.sendEventToWorkflowById);
  ...
  _getGatewayUrl() {
    const host = process.env.ZENATON_GATEWAY_URL ? process.env.ZENATON_GATEWAY_URL : ZENATON_GATEWAY_BASE_URL;
    return `${host}/api`;
  }
  ...
  _getError(err) {
    ...
    if (err.response && err.response.status >= 500) {
      return ["InternalZenatonError", `Please contact Zenaton support - ${err.message}`];
    }
